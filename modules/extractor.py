...
def split_by_low_regions(arr: numpy.ndarray, threshold: int = 128):
    """
    주어진 1D NumPy 배열을 threshold 이하 구간을 경계로 하여
    
    Args:
        arr: 1D NumPy array
        threshold: 경계로 사용할 값 (여기서는 128)
    
    Returns: 
        segements: arr에서 threshold 이하 구간을 경계로 하여
    """
    mask = arr > threshold
...
def calc_boundary(dicom_list: list):
    ...
    if len(dicom_list) > 0:
        ...
        mask = numpy.zeros_like(pixel_array, dtype=numpy.uint8)
        shift = 148
...
def remove_bottom_arc(pixel_array: numpy.ndarray, boundary: list):
    ...
    # temp_d = distance
    # if distance == h or distance < 400:
    #     distance += 36
    # if distance > 500:
    #     distance -= 32
    # # print(f"\n\n original: {temp_d}, distance: {distance}\n\n")
    # mask = numpy.zeros_like(pixel_array, dtype=numpy.uint8)
    # shift = 148
    # center = (target_x, -shift)
    # radius = distance + shift
...
def remove_skin(pixel_array: numpy.ndarray, contours: list, area: str, thickness: int = 16):
    """
    이미지에서 피부를 제거하는 함수
    ...
    back_line_points = [point for point in body_outline_contour if point[0][1] >= pixel_array.shape[0] // 2 + 96]
    cv2.drawContours(pixel_array, [body_outline_contour], -1, 0, 8, cv2.LINE_8) # 피부 제거
    for i in range(len(back_line_points) - 1): # 등 피부 제거
    ...
    middle_point = min(back_line_points, key=lambda point: abs(point[0][0] - w // 2))[0]
    middle_point[1] += 12
    bottom_points = []
    ...
    t_dx = right_point[0] - left_point[0]
    t_dy = right_point[1] - left_point[1]
    l_length = math.sqrt(l_dx**2 + l_dy**2)
    r_length = math.sqrt(r_dx**2 + r_dy**2)
    t_length = math.sqrt(t_dx**2 + t_dy**2)
    sin60 = math.sqrt(3) / 2
    cos60 = 0.5
    ...
    if area == "over":
        ...
        r_third_point_x2 = middle_point[0] + r_dx * cos60 + r_dy * sin60
        ...
        for x in range(left_x[0], left_x[1] + 1):
            ...
            dy = math.sqrt(l_length**2 - dx**2) if l_length**2 >= dx**2 else 0
            bottom_points.append((x, int(l_third_point[1] + dy)))

        for x in range(right_x[0], right_x[1] + 1):
            dx = x - r_third_point[0]
            dy = math.sqrt(r_length**2 - dx**2) if r_length**2 >= dx**2 else 0
            bottom_points.append((x, int(r_third_point[1] + dy)))
    else:
        # Calculate the circle center passing through left_point, middle_point, and right_point
        ...
        if G != 0:
            ...
            radius = int(math.sqrt((circle_center_x - left_point[0])**2 + (circle_center_y - left_point[1])**2))

            # cv2.circle(pixel_array, circle_center, radius, 3000, 2)
            
            for x in range(total_x[0], total_x[1] + 1):
                dx = x - circle_center[0]
                dy = math.sqrt(radius**2 - dx**2) if radius**2 >= dx**2 else 0
                bottom_points.append((x, int(circle_center[1] + dy)))
        
    bottom_points.append((w, h))
    bottom_points.append((0, h))
    
    cv2.fillPoly(pixel_array, [numpy.array(bottom_points)], 0)
    return pixel_array
...
def masking_bone_inner_area(pixel_array: numpy.ndarray, bone_inner_points: numpy.ndarray):
    ...
    bone_center_point = numpy.mean(numpy.argwhere(red_pixels), axis=0).astype(int)
    
    distance = numpy.mean(numpy.linalg.norm(bone_inner_points - bone_center_point, axis=2))
    dispersion = numpy.std(numpy.linalg.norm(bone_inner_points - bone_center_point, axis=2))
    distance = distance + dispersion

    target_bone_inner_points = [point for point in bone_inner_points if numpy.linalg.norm(point - bone_center_point) < distance]
...
def find_abs(norm_idx: float, pixel_array: numpy.ndarray, bone_contours: list, vertebrae_contour: list):
    ...
    Returns:
        num-py.ndarray: 복근 마스크
        dict: 복근 정보
    """
    red_pixels = (pixel_array[:, :, 0] == 255) & (pixel_array[:, :, 1] == 0) & (pixel_array[:, :, 2] == 0)
    top_y = numpy.where(red_pixels)[0].min()
    bottom_y = numpy.where(red_pixels)[0].max()
    ...
    _, w = pixel_array.shape[:2]
    center_point = tuple([int(vertebrae_center[0][0]), int(vertebrae_center[0][1])])
    try:
        front_left_rib_contour = [contour for contour in bone_contours if all(point[0][0] < pixel_array.shape[1] // 2 for point in contour)]
        front_left_rib_contour = sorted(front_left_rib_contour, key=lambda x: min(point[0][1] for point in x))[:1]
        front_right_rib_contour = [contour for contour in bone_contours if all(point[0][0] >= pixel_array.shape[1] // 2 for point in contour)]
        front_right_rib_contour = sorted(front_right_rib_contour, key=lambda x: min(point[0][1] for point in x))[:1]
        ...
        left_y_intercept = float(left_point[1] + (center_point[1] - left_point[1]) * (0 - left_point[0]) / (center_point[0] - left_point[0]))
        right_y_intercept = float(right_point[1] + (center_point[1] - right_point[1]) * (w - right_point[0]) / (center_point[0] - right_point[0]))
    except:
        left_point = (-1, -1)
        right_point = (-1, -1)
        left_y_intercept = cur_max_y
        right_y_intercept = cur_max_y
    abs_info = {"center": center_point, "left": left_point, "right": right_point, "left_y_intercept": left_y_intercept, "right_y_intercept": right_y_intercept}
    return abs_info
...
def masking_inner_area_from_lung(pixel_array: numpy.ndarray, contours: list, bone_contours: list, vertebrae_contour: numpy.ndarray):
    ...
    for point in sorted_boundary_points:
        closest_point = min(bone_points, key=lambda p: numpy.linalg.norm(numpy.array(p[0]) - numpy.array(point)))
        bone_inner_points.append(closest_point)
    
    # for p in bone_points:
    #     cv2.circle(pixel_array, (p[0][0], p[0][1]), 2, (255, 255, 255), -1)
    # cv2.ellipse(pixel_array, bone_center_point, (bone_w, bone_h), 0, 0, 360, (0, 255, 0), 1)
    # for p in sorted_boundary_points:
    #     cv2.circle(pixel_array, (p[0], p[1]), 2, (255, 100, 255), -1)
    # cv2.drawContours(pixel_array, [numpy.array(bone_inner_points)], -1, (0, 255, 255), 2)
    # for p in bone_inner_points:
    #     cv2.circle(pixel_array, (p[0][0], p[0][1]), 2, (255, 255, 0), -1)
...
def extract_muscle_from_over_lung(
    ...
    pixel_array = remove_skin(pixel_array, contours, "over", 20)
...
def extract_muscle_from_lung(
    ...
    contours = find_contours(normalized_pixel_array)
    normalized_pixel_array = fill_contours(normalized_pixel_array, contours)
    pixel_array = remove_skin(pixel_array, contours, "lung")
    ...
    normalized_pixel_array = masking_inner_area_from_lung(normalized_pixel_array, contours, bone_contours, vertebrae_contour)
    normalized_pixel_array = extract_muscle_only(normalized_pixel_array)
    return normalized_pixel_array
...
def extract_muscle_from_under_lung(
    ...
    contours = find_contours(normalized_pixel_array)    
    normalized_pixel_array = fill_contours(normalized_pixel_array, contours)
    pixel_array = remove_skin(pixel_array, contours, "under")
    muscle_mask = find_muscle(pixel_array, False)
    normalized_pixel_array = fill_muscle(normalized_pixel_array, muscle_mask)
    bone_mask, bone_contours = find_bone(pixel_array, muscle_mask)
    bone_inner_points = find_bone_inner_contours(bone_contours)
...
def extract(idx: int, norm_idx: int, category: str, sn: str, method: str, boundary: list, dicom_path: str, visualize_dir: str):
    """
    CT 이미지에서 타겟 영역(Rib Muscle)을 추출하는 함수

    Args:
        idx(int): 이미지 인덱스
        norm_idx (int): 정규화된 이미지 인덱스
        category (str): 이미지 카테고리
        sn (str): 시리즈 번호
        method (str): 추출 방법
        dicom_path (str): DICOM 파일 경로
    ...
    metadata["sternum"] = True if method == "upper" or method == "lung" else False
    if method == "upper":
        result = extract_muscle_from_over_lung(norm_idx, pixel_array, normalized_pixel_array)
    elif method == "lung":
        result = extract_muscle_from_lung(norm_idx, pixel_array, normalized_pixel_array)
    elif method == "lower":
        result, abs_info = extract_muscle_from_under_lung(norm_idx, pixel_array, normalized_pixel_array)
        metadata["abs"]["base_point"] = abs_info["center"]
        metadata["abs"]["left_point"] = abs_info["left"]
        metadata["abs"]["right_point"] = abs_info["right"]
        metadata["abs"]["left_y_intercept"] = abs_info["left_y_intercept"]
        metadata["abs"]["right_y_intercept"] = abs_info["right_y_intercept"]
    # if norm_idx < lung_area[0]:
    #     metadata["position"] = "upper"
    #     result = extract_muscle_from_over_lung(norm_idx, pixel_array, normalized_pixel_array)
    # else:
    #     flag = check_sternum(pixel_array)
    #     if norm_idx > lung_area[1]:
    #         flag = False
    #     if flag:
    #         metadata["position"] = "lung"
    #         metadata["sternum"] = True
    #         result = extract_muscle_from_lung(norm_idx, pixel_array, normalized_pixel_array)
    #     else:
    #         metadata["position"] = "lower"
    #         metadata["sternum"] = False
    #         result, abs_info = extract_muscle_from_under_lung(norm_idx, pixel_array, normalized_pixel_array)
    #         metadata["abs"]["base_point"] = abs_info["center"]
    #         metadata["abs"]["left_point"] = abs_info["left"]
    #         metadata["abs"]["right_point"] = abs_info["right"]
