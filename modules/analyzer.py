...
@log_execution_time
def load_label(checkpoint_dir: str, label_dir: str):
    """
    ...
    """
    def check_label_exist(in_path: str, ex_path: str):
        """
        ...
        Returns:
            bool: NRRD 파일 존재 여부
        """
        patient_id = in_path.split("/")[2]
        in_flag = os.path.isfile(in_path)
        ex_flag = os.path.isfile(ex_path)
    ...
    if check_checkpoint(checkpoint_path):
        return pickle.load(open(checkpoint_path, 'rb')), True
        ...
        for category in ["in", "ex"]:
            ...
            if os.path.isfile(nrrd_file):
                ...
                for segment in segmentation['segments']:
                    ...
                    if segment['name'].lower() in ["abdomen", "abs"]:
                        abdomen.append((segment['name'], segment['labelValue']))
                        abdomen_flag = True
                if len(target) > 0:
                    ...
                    for idx in range(num_slide - 1, -1, -1):
                        ...
                        for y in range(height):
                            for x in range(width):
                                ...
                                if flag:
                                    image[y, x] = [0, 0, 255]
                                if abdomen_flag and abdomen_voxels[x, y, idx] == 1:
                                    image[y, x] = [0, 0, 0]
                        black_pixels = numpy.all(image == [0, 0, 0], axis=-1).astype(numpy.uint8)
                        black_pixels = black_pixels * 255
                        num_labels, labels, stats, _ = cv2.connectedComponentsWithStats(black_pixels, connectivity=8)
                        ...
                        # plt.imsave(f"./temp/test_{category}_{idx}.png", temp_image)
            label_muscle_image_dict[category] = label_image_data
        pickle.dump(label_muscle_image_dict, open(checkpoint_path, 'wb'))
...
def calculate_iou(predict_image: numpy.ndarray, label_image: numpy.ndarray):
    ...
    Returns:
        float: IoU 값
    """
    def rgb_to_gray(image: numpy.ndarray):
        """
        ...
        """
        gray_image = numpy.where(numpy.all(image == [0, 0, 0], axis=-1), 0, 255).astype(numpy.uint8)
        return gray_image
...
def visualize(idx: int, category: str, pred_image: numpy.ndarray, label_image: numpy.ndarray, iou: float, output_dir: str):
    ...
    plt.title(f"Predict Image")
    plt.axis("off")
    overlap_image = cv2.addWeighted(pred_image.astype(numpy.float32), 0.5, label_image.astype(numpy.float32), 0.5, 0)
    overlap_image = numpy.clip(overlap_image, 0, 255).astype(numpy.uint8)
    plt.subplot(1, 3, 3)
    plt.imshow(overlap_image)
    plt.title(f"Overlap Image")
    plt.axis("off")
    plt.figtext(0.5, 0.1, f"Slide Idx: {idx}, IoU: {iou}", ha="center", fontsize=16)
    plt.savefig(os.path.join(cur_output_dir, f"{idx:03d}.png"))
    plt.close()
